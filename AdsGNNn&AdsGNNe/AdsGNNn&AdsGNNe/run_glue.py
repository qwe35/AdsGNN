...
"""
    Update for the ads relevance and CTR prediction task
"""
...
from data import glue_convert_examples_to_features as convert_examples_to_features
from data import glue_output_modes as output_modes
from data import glue_processors as processors
...
def train(args, train_dataset, model, tokenizer, tb_writer):
    ...
    for _ in train_iterator:
        ...
        #            os.path.join(output_dir, "model.bin"))  # save the model state_dict file
        tokenizer.save_pretrained(output_dir)
...
def generate_model_inputs(batch, FLAGS=None):
    inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3]}
    if FLAGS.model_type != "distilbert":
        inputs["token_type_ids"] = (
            batch[2] if FLAGS.model_type in ["bert", "xlnet", "albert"] else None
        )  # XLM, DistilBERT, RoBERTa, and XLM-RoBERTa don't use segment_ids
    inputs["row_id"] = batch[4]
    return inputs
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                # wrapper input for model
                model_inputs = {"input_ids": inputs["input_ids"], "attention_mask": inputs["attention_mask"], "labels": inputs["labels"]}
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, data_mode=None):
    ...
    all_row_id = torch.tensor([f.row_id for f in features], dtype=torch.long)

    # inputs = {"input_ids": batch[0], "attention_mask": batch[1], "token_type_ids": batch[2], "labels": batch[3]}
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_row_id)
    return dataset
...
def main():
    ...
    parser.add_argument("--evaluate_every_epoch", type=bool, default=True, help="evaluate every epoch")
    ...
    def get_current_model_config_from_args(current_args):
        tmp_config = config_class.from_pretrained(
            current_args.config_name if current_args.config_name else current_args.model_name_or_path,
            num_labels=num_labels,
            finetuning_task=current_args.task_name,
            cache_dir=current_args.cache_dir if current_args.cache_dir else None,
        )
        return tmp_config
