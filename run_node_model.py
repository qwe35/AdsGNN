...
use pretrained bert-based-uncased
    use the transformers bert version https://github.com/huggingface/transformers
node-level gnn model for BertGNN project
- update args
- add node-level convert_to_Tensors
- add node-level generate_model_inputs
- use node-level BertGNN model
"""
...
from data import qkgnn_convert_examples_to_node_level_features as convert_examples_to_features
...
from model import BertNodeGnnModel, BertNodeGnnConfig
...
MODEL_CLASSES = {
    ...
    "graphsage" : (BertNodeGnnConfig, BertNodeGnnModel, BertTokenizer),
    "gat" : (BertNodeGnnConfig, BertNodeGnnModel, BertTokenizer),
}
...
def convert_to_Tensors(features, output_mode):
    ...
    input_ids_k1 = torch.tensor([f.input_ids_k1 for f in features], dtype=torch.long)
    input_mask_k1 = torch.tensor([f.input_mask_k1 for f in features], dtype=torch.long)
    segment_ids_k1 = torch.tensor([f.segment_ids_k1 for f in features], dtype=torch.long)

    input_ids_k2 = torch.tensor([f.input_ids_k2 for f in features], dtype=torch.long)
    input_mask_k2 = torch.tensor([f.input_mask_k2 for f in features], dtype=torch.long)
    segment_ids_k2 = torch.tensor([f.segment_ids_k2 for f in features], dtype=torch.long)

    input_ids_k3 = torch.tensor([f.input_ids_k3 for f in features], dtype=torch.long)
    input_mask_k3 = torch.tensor([f.input_mask_k3 for f in features], dtype=torch.long)
    segment_ids_k3 = torch.tensor([f.segment_ids_k3 for f in features], dtype=torch.long)

    input_ids_q1 = torch.tensor([f.input_ids_q1 for f in features], dtype=torch.long)
    input_mask_q1 = torch.tensor([f.input_mask_q1 for f in features], dtype=torch.long)
    segment_ids_q1 = torch.tensor([f.segment_ids_q1 for f in features], dtype=torch.long)

    input_ids_q2 = torch.tensor([f.input_ids_q2 for f in features], dtype=torch.long)
    input_mask_q2 = torch.tensor([f.input_mask_q2 for f in features], dtype=torch.long)
    segment_ids_q2 = torch.tensor([f.segment_ids_q2 for f in features], dtype=torch.long)

    input_ids_q3 = torch.tensor([f.input_ids_q3 for f in features], dtype=torch.long)
    input_mask_q3 = torch.tensor([f.input_mask_q3 for f in features], dtype=torch.long)
    segment_ids_q3 = torch.tensor([f.segment_ids_q3 for f in features], dtype=torch.long)
...
def generate_model_inputs(batch, FLAGS=None):
    features = {
        ...
        "input_ids_k1": batch[9], "input_mask_k1": batch[10], "segment_ids_k1": batch[11],
        "input_ids_k2": batch[12], "input_mask_k2": batch[13], "segment_ids_k2": batch[14],
        "input_ids_k3": batch[15], "input_mask_k3": batch[16], "segment_ids_k3": batch[17],
        "input_ids_q1": batch[18], "input_mask_q1": batch[19], "segment_ids_q1": batch[20],
        "input_ids_q2": batch[21], "input_mask_q2": batch[22], "segment_ids_q2": batch[23],
        "input_ids_q3": batch[24], "input_mask_q3": batch[25], "segment_ids_q3": batch[26],
    ...
    q_neigh = [
        features["input_ids_k1"], features["input_mask_k1"], features["segment_ids_k1"],
        features["input_ids_k2"], features["input_mask_k2"], features["segment_ids_k2"],
        features["input_ids_k3"], features["input_mask_k3"], features["segment_ids_k3"]]
    k_neigh = [
        features["input_ids_q1"], features["input_mask_q1"], features["segment_ids_q1"],
        features["input_ids_q2"], features["input_mask_q2"], features["segment_ids_q2"],
        features["input_ids_q3"], features["input_mask_q3"], features["segment_ids_q3"]]
...
def main():
    ...
    parser.add_argument("--use_kkqq", type=lambda s: s.lower() in ['true', 't', 'yes', '1', '', None], default=False,
    ...
    parser.add_argument("--gnn_token_embedding_path", type=str, default="leaky_relu",
    ...
    # args for node-level model
    parser.add_argument("--gnn_aggregator", type=str, default="meanpool", help="gnn_aggregator fro graphsage model")
    parser.add_argument("--gnn_head_nums", type=lambda s: list(map(int, s.strip().split(';'))), default=[8],
                        help="gnn head nums for each layer, split by ;")
    parser.add_argument("--gnn_residual", type=str, default="concat", help="add or concat")

    # is_use_gnn
    parser.add_argument("--is_use_gnn", type=lambda s: s.lower() in ['true', 't', 'yes', '1', '', None], default=True,
                        help="is_use_gnn mode")
    ...
    def get_current_model_config_from_args(current_args):
        ...
        tmp_config = config_class(
            gnn_encoder=current_args.model_type,
            gnn_aggregator=current_args.gnn_aggregator,
            gnn_head_nums=current_args.gnn_head_nums,
            num_labels=num_labels,
            bert_model_path_or_name=current_args.model_name_or_path,
            gnn_fanouts=[3],
            gnn_residual=current_args.gnn_residual,
            is_freeze_bert=current_args.is_freeze_bert,
            inference_neighbor_bert=current_args.inference_neighbor_bert,
            is_use_gnn=current_args.is_use_gnn
